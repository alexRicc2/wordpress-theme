...
function the_post_custom_thumbnail(
  $post_id,
...
function aquila_posted_on()
{
  ...
  if (get_the_time('U') !== get_the_modified_time('U')) {
    $time_string = '<time class="entry-date published" datetime="%1$s">Posted on %2$s</time><time class="updated " datetime="%3$s">%4$s</time>';
  }
  $time_string = printf(
    $time_string,
...
function aquila_excerpt_more($more=''){
  ...
  if(!is_single()){
    $more = sprintf('<br><button class=" btn btn-info"><a class="aquila-read-more text-white" href="%1$s">%2$s</a></button>',get_permalink(get_the_ID()), __('Read more', 'aquila'));
  }
